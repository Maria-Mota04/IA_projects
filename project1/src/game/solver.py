...
class Solver:
    ...
    def ensure_pdb(self):
        if self._patterns is None:
            self._patterns = build_patterns(self._n, self._group_size)

        if self._pdb_5 is None:
            print(f"[PDB] A gerar PDB para {self._group_size} peças... (n={self._n}, k={self._k})")
            self._pdb_5 = generate_pdb(self._n, self._k, self._group_size)
            print("[PDB] Gerada com sucesso!")

    def has_pdb(self) -> bool:
        return self._patterns is not None and self._pdb_5 is not None
    ...
    def solve(
        ...
        if mode == gameMode.NORMAL_GAME:
            return self._run_normal_game(game, screen)
    ...
    def _run_normal_game(self, game: Game, screen):
        game_running = True
        ...
        while game_running:
            ...
            for event in pygame.event.get():
                ...
                if event.type == pygame.MOUSEBUTTONUP:
                    click_pos = event.pos
                    dist = math.sqrt((click_pos[0] - center_circle[0]) ** 2 + (click_pos[1] - center_circle[1]) ** 2)
                    ...
                    if dist <= radius_circle:
                    ...
                    elif settings_button_hint.collidepoint(click_pos):
                        ...
                        if hinted_state:
                            ...
                            hint_highlight = {
                                i for i in range(len(c_tiles))
                                if c_tiles[i] != h_tiles[i]
                            }
                        else:
                            hint_highlight = None
                    ...
                    elif control_helper_menu_button.collidepoint(click_pos):
